(function() {
  ...
  const wrappedDesc = wrapTooltipText(descText, 50) || '???';
    ...
    if (costIcon) {
      try {
        const src = chrome.runtime && chrome.runtime.getURL ? chrome.runtime.getURL('icons/thumb_down.png') : 'icons/thumb_down.png';
        costIcon.src = src;
      } catch (_) {}
    }
    if (desc) desc.textContent = wrappedDesc;
    if (state.tipEl) {
