(function(){
  ...
  (function addSlotMachine(){
    ...
    const GOOD_IMAGES = ['icons/clicker/good1.png', 'icons/clicker/good2.png', 'icons/clicker/good3.png', 'icons/clicker/good4.png'];
    const BAD_IMAGES = ['icons/clicker/bad1.png', 'icons/clicker/bad2.png', 'icons/clicker/bad3.png', 'icons/clicker/bad4.png'];
    const SUPER_IMAGE = 'icons/clicker/deploy_jettom.png';
    const ALL_IMAGES = GOOD_IMAGES.concat(BAD_IMAGES).concat([SUPER_IMAGE]);
    const SPIN_SFX = 'icons/clicker/slot_pull.mp3';
    const CLUNK_SFX = 'icons/clicker/clunk.mp3';
    ...
    spinImg.src = chrome.runtime.getURL ? chrome.runtime.getURL('icons/clicker/spin.png') : 'icons/clicker/spin.png';
    ...
    const applyResult = (result)=>{
      try { console.log('[FJFE Slot] result:', result); } catch(_) {}
      ...
      if (result === 'good') {
        ...
        try { console.log('[FJFE Slot] good roll:', roll); } catch(_) {}
        if (roll < 0.50) {
          try { console.log('[FJFE Slot] good outcome: front-page'); } catch(_) {}
          const cur = getMoney();
        ...
        if (roll < 0.75) {
          try { console.log('[FJFE Slot] good outcome: mod-help'); } catch(_) {}
        ...
        if (roll < 0.95) {
          try { console.log('[FJFE Slot] good outcome: overflow'); } catch(_) {}
          const candidates = getProducerCounts().filter(p => p.count >= 10);
          if (!candidates.length) {
            try { console.log('[FJFE Slot] overflow fallback: front-page'); } catch(_) {}
            const cur = getMoney();
            const gainA = Math.floor(cur * 0.15);
            const gainB = Math.floor(rpsVal * 900);
            const gain = Math.max(0, Math.min(gainA, gainB));
        ...
        try { console.log('[FJFE Slot] good outcome: containment'); } catch(_) {}
        {
      ...
      if (result === 'bad') {
        if (Math.random() < 0.50) {
          try { console.log('[FJFE Slot] bad outcome: nsfw'); } catch(_) {}
          const cur = getMoney();
        ...
        try { console.log('[FJFE Slot] bad outcome: ban'); } catch(_) {}
      ...
      if (result === 'super') {
        if (Math.random() < 0.50) {
          try { console.log('[FJFE Slot] super outcome: jettom'); } catch(_) {}
          const dur = adjustGoodDuration(60);
        ...
        try { console.log('[FJFE Slot] super outcome: dj-admin'); } catch(_) {}
        {
      ...
      try { console.log('[FJFE Slot] outcome: none'); } catch(_) {}
    ...
    const updateTooltip = ()=>{
      ...
      const imageSrc = spinImg && spinImg.src ? spinImg.src : (chrome.runtime.getURL ? chrome.runtime.getURL('icons/clicker/spin.png') : 'icons/clicker/spin.png');
