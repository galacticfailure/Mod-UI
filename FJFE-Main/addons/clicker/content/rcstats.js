(function(){
  ...
  function statRow(label, value, opts={}){
    ...
    if (opts.thumb || opts.iconPath){
      ...
      const iconPath = opts.iconPath || 'addons/clicker/icons/thumb.png';
  ...
  function actuallyOpenMenu(){
    ...
    list.appendChild(statRow('Alts:', '', {
      iconPath:'addons/clicker/icons/alts.png',
      valueBuilder: (val) => {
  ...
  function addStatsButton(host){
    ...
    const img = document.createElement('img');
    const path = 'addons/clicker/icons/clickerinfo.png';
  ...
  function refreshPurchasedGrid(){
    try {
      ...
      purchased.forEach(def => {
        ...
        let iconPath;
        const mutIconMap = {
          mut1: 'addons/clicker/icons/lolcat.png',
          mut2: 'addons/clicker/icons/alpha.png',
          mut3: 'addons/clicker/icons/facebook.png',
          mut4: 'addons/clicker/icons/tragedy.png',
          mut5: 'addons/clicker/icons/vidya.png',
          mut6: 'addons/clicker/icons/foreign.png',
          mut7: 'addons/clicker/icons/fj.png',
        };
        if (def.icon) {
          iconPath = def.icon;
          ...
          iconPath = 'addons/clicker/icons/time.png';
        } else if (/^amntt\d+$/.test(def.id)) {
          iconPath = def.id === 'amntt1' ? 'addons/clicker/icons/offline.png' : 'addons/clicker/icons/amount.png';
        } else if (/^mbt\d+$/.test(def.id)) {
          iconPath = 'addons/clicker/icons/meme.png';
        } else if (/^lct\d+$/.test(def.id)) {
          iconPath = 'addons/clicker/icons/lolcat.png';
        } else if (/^gat\d+$/.test(def.id)) {
          iconPath = 'addons/clicker/icons/alpha.png';
        } else if (/^fbt\d+$/.test(def.id)) {
          iconPath = 'addons/clicker/icons/facebook.png';
        } else if (/^tgt\d+$/.test(def.id)) {
          iconPath = 'addons/clicker/icons/tragedy.png';
        } else if (/^vgt\d+$/.test(def.id)) {
          iconPath = 'addons/clicker/icons/vidya.png';
        } else if (/^fgt\d+$/.test(def.id)) {
          iconPath = 'addons/clicker/icons/foreign.png';
        } else if (/^fjt\d+$/.test(def.id)) {
          iconPath = 'addons/clicker/icons/fj.png';
        } else if (/^spt\d+$/.test(def.id)) {
          iconPath = 'addons/clicker/icons/special.png';
        } else if (/^slott\d+$/.test(def.id)) {
          iconPath = 'addons/clicker/icons/spin.png';
        } else if (def.currency === 'alts' && def.producerId) {
          const baseName = (def.producerId || '')
            .replace(/([a-z0-9])([A-Z])/g, '$1 $2')
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, '_');
          iconPath = `addons/clicker/icons/production/${baseName}.png`;
        } else if (/^clickt\d+$/.test(def.id)) {
          iconPath = 'addons/clicker/icons/upgrades/clickup.png';
        } else {
          const m = def.id.match(/^(.+?)t(\d+)$/);
          const producerId = m ? m[1] : '';
          iconPath = producerId ? `addons/clicker/icons/upgrades/${producerId}up.png` : 'addons/clicker/icons/upgrades/clickup.png';
        }
