(function() {
  ...
  const imgPath = def && def.img ? def.img : `addons/clicker/icons/production/${defaultImgName}.png`;
  ...
  priceNum.textContent = computeUnlocked() ? formatPriceAny(calcPrice()) : '???';
    ...
    priceIcon.src = chrome.runtime.getURL ? chrome.runtime.getURL('addons/clicker/icons/thumb_down.png') : 'addons/clicker/icons/thumb_down.png';
    priceIcon.onerror = () => {
  ...
  const defaultImgName2 = (def.name || id).toLowerCase().replace(/[^a-z0-9]+/g, '_');
    const imgPath2 = def.img ? def.img : `addons/clicker/icons/production/${defaultImgName2}.png`;
  ...
  priceNum.textContent = formatPriceAny(calcPrice());
        ...
        name.textContent = '???';
        const unk = 'addons/clicker/icons/production/unknown.png';
