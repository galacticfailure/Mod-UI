(() => {
	...
	const renderCookButton = (meta) => {
		...
		if (data && data.item && data.count > 0) {
			...
			img.decoding = 'async';
			img.loading = 'lazy';
			img.src = resolve(`addons/farm/icons/food/${data.item}.png`);
	...
	const handleCookSlotClick = (e, meta, isRightClick) => {
		...
		if (slot && slot.item && slot.count > 0) {
			...
			const selection = {
				...
				itemType: 'food',
				type: 'inventory-item',
				icon: `addons/farm/icons/food/${picked.item}.png`
			};
	...
	const buildLockedSection = (sectionName) => {
		...
		lockImg.src = resolve('addons/farm/icons/locked.png');
		...
		coin.src = resolve('addons/farm/icons/coin.png');
	...
	const buildPlusGrid=(sectionName)=>{
		...
		bgImg.loading = 'lazy';
		bgImg.src = (typeof chrome !== 'undefined' && chrome.runtime?.getURL) ? chrome.runtime.getURL(`addons/farm/icons/${toKey(sectionName)}.png`) : `addons/farm/icons/${toKey(sectionName)}.png`;
	...
	const buildUI=()=>{
		...
		rbIcon.loading = 'lazy';
		rbIcon.src = resolve('addons/farm/icons/recipe_book.png');
			...
			try {
				...
				if (recipeReplacingInventory) {
					...
					if (recipePanelEl) {
						...
						rbIcon.src = resolve('addons/farm/icons/inventory.png');
					}
				}
			} catch(_) {}
		});
	...
	const getTooltipDataForTokenKey = (key) => {
		...
		if (ingEntry) {
			...
			return {
				imageSrc: resolve(`addons/farm/icons/food/${toFoodKey(ingEntry.name)}.png`),
				name: ingEntry.name,
		...
		if (recEntry) {
			...
			return {
				imageSrc: resolve(`addons/farm/icons/food/${toFoodKey(recEntry.name)}.png`),
				name: recEntry.name,
		...
		return { imageSrc: resolve(`addons/farm/icons/food/${norm}.png`), name: key };
	};
	...
	const updateCraftPreviewForSection = (sectionName) => {
		...
		img.src = resolve(`addons/farm/icons/food/${key}.png`);
		img.onerror = function(){ this.src = resolve('icons/error.png'); };
	...
	const selection = { key: productKey, item: productKey, count: 1, itemType: 'food', type: 'inventory-item', icon: `addons/farm/icons/food/${productKey}.png` };
		interact.selectItem?.(selection, null);
	};
	...
	const buildListEntry = (entry, isRecipeEntry = false) => {
		...
		titleImg.src = hasCrafted ? resolve(`addons/farm/icons/food/${entryKey}.png`) : resolve('addons/farm/icons/unknown.png');
		...
		if (Array.isArray(entry.ing) && entry.ing.length) {
			...
			toks.forEach(tok => {
				...
				i.src = seen ? resolve(`addons/farm/icons/food/${keyNorm}.png`) : resolve('addons/farm/icons/unknown.png');
		...
		const showEntryTooltip = () => {
			try {
				...
				const data = {
					imageSrc: resolve(`addons/farm/icons/food/${toFoodKey(entry.name||'')}.png`),
					name: entry.name || '',
					bodyTT: entry.desc || '',
					cost: String(price),
					costIcon: 'addons/farm/icons/coin.png'
		...
		if (!hasSeenAll) {
			...
			overlay.src = resolve('addons/farm/icons/unknown.png');
