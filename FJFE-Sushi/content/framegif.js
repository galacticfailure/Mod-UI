(() => {
  ...
  const MEDIA_BUTTON_OFFSET = 6;

  const getButtonContainer = (node) => {
    if (!node) return null;
  ...
  const createMediaButton = (node) => {
    ...
    button.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopPropagation();
      openMenuForTarget(node);
    });
    return button;
  };

  const positionMediaButton = (node, button, container) => {
    ...
    if (container === document.body) {
      const top = window.scrollY + rect.top + MEDIA_BUTTON_OFFSET;
      const left = window.scrollX + rect.left + MEDIA_BUTTON_OFFSET;
      button.style.top = `${top}px`;
      button.style.left = `${left}px`;
    } else {
      button.style.top = `${MEDIA_BUTTON_OFFSET}px`;
      button.style.left = `${MEDIA_BUTTON_OFFSET}px`;
    }
  };

  const ensureMediaButton = (node) => {
    if (!node) return;
    const existing = mediaButtons.get(node);
    if (existing && document.contains(existing)) {
      const container = existing.parentElement;
      positionMediaButton(node, existing, container);
      return;
    }
    const container = getButtonContainer(node);
    if (!container) return;
    const button = createMediaButton(node);
    container.appendChild(button);
    mediaButtons.set(node, button);
    positionMediaButton(node, button, container);
  };

  const refreshMediaButtons = () => {
    const nodes = Array.from(document.querySelectorAll('img,video'))
      .filter((node) => getMediaKind(node) && isIncludedMediaTarget(node));
    const live = new Set(nodes);
    nodes.forEach(ensureMediaButton);
    Array.from(mediaButtons.entries()).forEach(([node, button]) => {
      if (!live.has(node) || !node.isConnected) {
        try { button.remove(); } catch (_) {}
        mediaButtons.delete(node);
      }
    });
  };

  const clearMediaButtons = () => {
    Array.from(mediaButtons.values()).forEach((button) => {
      try { button.remove(); } catch (_) {}
    });
    mediaButtons.clear();
  };
  ...
  const attachListeners = () => {
    ...
    if (!mediaObserver && typeof MutationObserver !== 'undefined') {
      ...
      if (document.body) {
        mediaObserver.observe(document.body, { childList: true, subtree: true, attributes: true });
      }
    }
    refreshMediaButtons();
  };
