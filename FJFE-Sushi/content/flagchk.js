(() => {
  ...
  const FLAG_MENU_HISTORY_PATH = '/ajax/getUserFlagHistory';
  ...
  let flagMenuFetchPromise = null;
  let flagMenuSummary = null;
  let flagMenuError = null;
  ...
  const updateFlagHistoryTarget = (context = {}) => {
    ...
    if (changed) {
      ...
      flagMenuSummary = null;
      flagMenuError = null;
      flagMenuFetchPromise = null;
  ...
  const normalizePremiumHistoryRecords = (records) => {
    ...
    return records.reduce((normalized, record) => {
      const action = (record?.category || record?.action || record?.type || '').trim().toLowerCase();
      if (!ACTION_ALLOW_LIST.has(action)) {
        return normalized;
      }
      const dateValue = record?.date || record?.time || record?.created || record?.created_at || '';
      const date = parseDateTime(trimText(dateValue));
      if (!date) {
        return normalized;
      }
      const timestamp = date.getTime();
      const isOld = timestamp < cutoffAllTime;
      const description = trimText(record?.info || record?.reason || record?.flag_reason || record?.note || '');
      const descLower = description.toLowerCase();
  ...
  const isFlagMenuRecord = (record) =>
    record &&
    typeof record === 'object' &&
    (Object.prototype.hasOwnProperty.call(record, 'info') ||
      Object.prototype.hasOwnProperty.call(record, 'category') ||
      Object.prototype.hasOwnProperty.call(record, 'date'));

  const isRecordArray = (records) => Array.isArray(records) && records.length && records.every(isFlagMenuRecord);

  const extractFlagMenuRecords = (payload) => {
    if (!payload) {
      return [];
    }
    if (Array.isArray(payload)) {
      ...
      if (payload.length === 1 && payload[0] && typeof payload[0] === 'object') {
        const nested = extractFlagMenuRecords(payload[0]);
        if (nested.length) {
          return nested;
        }
      }
      if (payload.length && payload.every((value) => typeof value === 'string')) {
        const collected = [];
        for (const value of payload) {
          try {
            const parsed = JSON.parse(value);
            const nested = extractFlagMenuRecords(parsed);
            if (nested.length) {
    ...
    if (typeof payload === 'string') {
      try {
        ...
        return extractFlagMenuRecords(parsed);
      } catch (_) {
        return [];
      }
    }
    if (Array.isArray(payload.flag_history)) {
      return payload.flag_history;
    }
    if (typeof payload.flag_history === 'string') {
      try {
        const parsed = JSON.parse(payload.flag_history);
        return extractFlagMenuRecords(parsed);
      } catch (_) {
        return [];
      }
    }
    if (Array.isArray(payload.flagHistory)) {
      return payload.flagHistory;
    }
    if (Array.isArray(payload.history)) {
      return payload.history;
    }
    if (payload.flag_history && typeof payload.flag_history === 'object') {
      const values = Object.values(payload.flag_history);
      if (values.length && values.every((value) => value && typeof value === 'object')) {
        return values;
      }
    }
    const nestedCandidates = [payload.data, payload.result, payload.payload].filter(Boolean);
    for (const candidate of nestedCandidates) {
      if (Array.isArray(candidate?.flag_history)) {
        return candidate.flag_history;
      }
      if (Array.isArray(candidate?.history)) {
        return candidate.history;
      }
      if (Array.isArray(candidate?.flagHistory)) {
        return candidate.flagHistory;
      }
    }
    if (payload && typeof payload === 'object') {
      const values = Object.values(payload);
      if (values.length && values.every((value) => value && typeof value === 'object' && ('date' in value || 'info' in value))) {
        return values;
      }
    }
    return [];
  ...
  const findFlagMenuRecordsDeep = (payload, depth = 0, seen = new Set()) => {
    if (!payload || depth > 4) {
      return [];
    }
    if (typeof payload === 'string') {
      try {
        const parsed = JSON.parse(payload);
        return findFlagMenuRecordsDeep(parsed, depth + 1, seen);
      } catch (_) {
        return [];
      }
    }
    ...
    const direct = extractFlagMenuRecords(payload);
    if (isRecordArray(direct)) {
      return direct;
    }

    if (Array.isArray(payload)) {
      for (const item of payload) {
        const found = findFlagMenuRecordsDeep(item, depth + 1, seen);
        if (found.length) {
          return found;
        }
      }
    ...
    for (const value of Object.values(payload)) {
      const found = findFlagMenuRecordsDeep(value, depth + 1, seen);
      if (found.length) {
        return found;
      }
    }
  ...
  const buildSummaryFromCounts = (counts = {}) => {
    const gore = normalizeCountValue(counts.gore) ?? 0;
    const borderline = normalizeCountValue(counts.borderline) ?? normalizeCountValue(counts.borderline_nsfw) ?? 0;
    const illegal = normalizeCountValue(counts.illegal) ?? 0;
    const nsfw = normalizeCountValue(counts.nsfw) ?? 0;
    const total = normalizeCountValue(counts.total) ?? gore + borderline + illegal + nsfw;
    return {
      total,
      ignored: 0,
      ignoredReasons: [],
      categories: {
        shi: { count: illegal, reasons: illegal ? Array(illegal).fill('Illegal') : [] },
        blGore: {
          count: gore + borderline,
          reasons: [
            ...Array(gore).fill('Gore'),
            ...Array(borderline).fill('Borderline NSFW')
          ]
        },
        other: { count: nsfw, reasons: nsfw ? Array(nsfw).fill('Other') : [] }
      }
    };
  };

  const extractFlagMenuCounts = (payload, depth = 0, seen = new Set()) => {
    if (!payload) {
      return null;
    }
    if (typeof payload === 'string') {
      try {
        const parsed = JSON.parse(payload);
        return extractFlagMenuCounts(parsed, depth + 1, seen);
      } catch (_) {
        return null;
      }
    }
    ...
    const isCountContainer = (value) =>
      value &&
      typeof value === 'object' &&
      (Object.prototype.hasOwnProperty.call(value, 'gore') ||
        Object.prototype.hasOwnProperty.call(value, 'borderline') ||
        Object.prototype.hasOwnProperty.call(value, 'borderline_nsfw') ||
        Object.prototype.hasOwnProperty.call(value, 'illegal') ||
        Object.prototype.hasOwnProperty.call(value, 'nsfw'));

    if (isCountContainer(payload)) {
      return payload;
    }
    if (typeof payload === 'object') {
      if (seen.has(payload)) {
    ...
    const nestedCandidates = [payload.data, payload.result, payload.payload, payload.counts, payload.stats].filter(Boolean);
    for (const candidate of nestedCandidates) {
      if (isCountContainer(candidate)) {
        return candidate;
      }
    }
    if (payload && typeof payload === 'object') {
      for (const value of Object.values(payload)) {
        if (isCountContainer(value)) {
          return value;
        }
        const nested = extractFlagMenuCounts(value, depth + 1, seen);
        if (nested) {
          return nested;
        }
      }
  ...
  const summarizeFlagMenuEntries = (entries) => {
    if (!Array.isArray(entries) || !entries.length) {
      return createEmptySummary();
    }
  ...
  const fetchFlagMenuSummary = async (ownerId) => {
    if (!ownerId) {
      throw new Error('Missing user context for history request.');
    }
    const base = window.location?.origin || `https://${targetHost}`;
    const url = new URL(FLAG_MENU_HISTORY_PATH, base);
    const body = new URLSearchParams({ commId: String(ownerId), uid: String(ownerId) });
    ...
    try {
      ...
      console.error('[fjfe flagchk] Flag menu response was not valid JSON.', { rawBody });
      throw new Error('History response was not valid JSON');
    }
    if (typeof parsed === 'string') {
      try {
        ...
        console.error('[fjfe flagchk] Flag menu response JSON contained a non-JSON string.', { parsed });
      }
    }
    const extracted = findFlagMenuRecordsDeep(parsed);
    if (!Array.isArray(extracted) || extracted.length === 0) {
      const counts = extractFlagMenuCounts(parsed);
      if (counts) {
      ...
      console.error('[fjfe flagchk] Flag menu response contained no flag history entries.', {
        parsed,
        summary,
        rawPreview: rawBody ? rawBody.slice(0, 240) : null
      });
      throw new Error('History response contained no flag history entries');
    }
    const entries = normalizePremiumHistoryRecords(extracted);
    return summarizeFlagMenuEntries(entries);
  };
  ...
  const ensureFlagMenuSummary = () => {
    const ownerId = flagHistoryTargetOwnerId;
    if (!ownerId) {
      const error = new Error('Unable to determine flagged user.');
      flagMenuError = error;
      updateFlagBreakdownDisplays(null, error);
      console.error('[fjfe flagchk] Flag menu summary missing user context.', error);
      return Promise.reject(error);
    }
    if (flagMenuSummary) {
      updateFlagBreakdownDisplays(flagMenuSummary);
      return Promise.resolve(flagMenuSummary);
    }
    if (flagMenuFetchPromise) {
      setFlagBreakdownLoading();
      return flagMenuFetchPromise;
    }
    flagMenuError = null;
    setFlagBreakdownLoading();
    const requestVersion = flagHistoryTargetVersion;
    flagMenuFetchPromise = fetchFlagMenuSummary(ownerId)
      .then((summary) => {
        if (requestVersion !== flagHistoryTargetVersion) {
          return summary;
        }
        flagMenuSummary = summary;
        flagMenuError = null;
        updateFlagBreakdownDisplays(summary);
        return summary;
      })
      .catch((error) => {
        ...
        console.error('[fjfe flagchk] Flag menu summary fetch failed.', error);
        throw error;
      })
      .finally(() => {
        if (requestVersion === flagHistoryTargetVersion) {
  ...
  const buildBoogAction = () => {
    ...
    return createChecklistAction('Receive verification from 4 Highish mods or 2 HC to flag. Use the ban time they recommend; baseline 100x hours.');
  };
  ...
  const CHECKLIST_FLOW = {
    ...
    QTP2: {
      ...
      options: [
        ...
        { label: 'Loli', next: 'QLTP' },
        { label: 'Boog', outcome: buildBanAction(15) },
    ...
    QLTP: {
      ...
      prompt: 'Take it to #mod-lolis, use ban time recommended by NSFW mods; if ban time due to past flags is higher, use that instead.',
      buttons: [
        { label: 'Got it', outcome: buildBanAction(15) }
      ]
    }
  };
  ...
  const ensureFlagInfoBreakdown = (form) => {
    ...
    if (flagMenuSummary) {
      updateFlagBreakdownDisplays(flagMenuSummary);
    } else if (flagMenuError) {
      updateFlagBreakdownDisplays(null, flagMenuError);
    } else {
      ensureFlagMenuSummary();
    }
  };
