  ...
  const MOBILE_DISABLE_SHORTCUT_HOTKEYS = true;
  ...
  function applyCustomShortcutsHijack() {
    ...
    (function ensureShortKeyCenteringStyles() {
      ...
      style.id = STYLE_ID;
      style.textContent = `
        /* Only horizontal centering; no font/size/vertical changes (mobile) */
        .ctButton4.mobQuickRate, #skinGuide.ctButton4 { position: relative; }
        .ctButton4.mobQuickRate .shortKey, #skinGuide.ctButton4 .shortKey {
          position: absolute;
          left: 50%;
  ...
  if (mobileBtns.length >= 6) {
    ...
    console.debug('[sccustom] getCustomShortcutButtons: No mobQuickRate quickM buttons found');
  ...
  icon.src = (typeof chrome !== 'undefined' && chrome?.runtime?.getURL) ? chrome.runtime.getURL('icons/reset.png') : 'icons/reset.png';
  icon.alt = 'Reset';
...
(() => {
  ...
  if (!MOBILE_DISABLE_SHORTCUT_HOTKEYS && !window.fjCustomShortcutsNumpadBound) {
    try {
