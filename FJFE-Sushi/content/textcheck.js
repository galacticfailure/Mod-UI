(() => {
  ...
  const getSanitizedText = (element) => {
    try {
      const clone = element.cloneNode(true);
      const mentionNodes = clone.querySelectorAll('.mlCom, [data-mlid]');
  ...
  const scanElement = (element) => {
    const sanitizedText = getSanitizedText(element);
    if (!sanitizedText) {
    ...
    PC_LIST.forEach(word => {
      if (text.includes(word.toLowerCase())) {
        ...
        const originalMatches = sanitizedText.match(regex);
        if (originalMatches) {
