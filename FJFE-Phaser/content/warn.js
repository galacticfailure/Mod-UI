(() => {
  ...
  const hasRecentVoteElement = () => !!document.getElementById('whoIsRating');
  ...
  const createOverlay = () => {
    ...
    scroller.textContent = MESSAGE_TEXT + '          ' + MESSAGE_TEXT;
  ...
  const resolved = (typeof text === 'string' && text.trim().length > 0) ? text : 'No misrate issues detected.';
  ...
  const positionWarningBadge = () => {
    ...
    let target = document.querySelector(OTHER_MEMES_SELECTOR) || document.querySelector('span[data-id="6"]');
    let container = target?.parentNode || document.getElementById('catControls') || document.querySelector('#catControls') || null;
    ...
    try {
      if (target && target.parentNode) {
        if (target.nextSibling !== warningBadge) target.parentNode.insertBefore(warningBadge, target.nextSibling);
      } else if (container) {
        if (warningBadge.parentNode !== container) container.appendChild(warningBadge);
      }
    } catch (_) {}
  ...
  const tickMarquee = (ts) => {
    if (!marqueeState || !overlayActive || !overlayElement) return;
  ...
  document.addEventListener('fjTweakerSettingsChanged', () => { evaluateState(); });
